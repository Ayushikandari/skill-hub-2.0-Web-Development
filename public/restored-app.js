...
const authAboutButton = document.querySelector("#auth-about-button");
const authHelpButton = document.querySelector("#auth-help-button");
const dashboardAboutButton = document.querySelector("#dashboard-about-button");
const dashboardHelpButton = document.querySelector("#dashboard-help-button");
...
const teamContacts = [
  { name: "Ayushi Kandari", email: "ayushikandari07@gmail.com" },
  { name: "Abhay Chauhan", email: "abhaychauhan2805@gmail.com" },
  { name: "Suhani Saklani", email: "suhanisaklani@gmail.com" },
  { name: "Vanshika Panwar", email: "vanshikapanwar11@gmail.com" }
];
...
function openInfoModal(kind) {
  const existing = document.querySelector(".info-modal-shell");
  if (existing) existing.remove();

  const isAbout = kind === "about";
  const title = isAbout ? "About SkillHub Engineering" : "Help and Contact";
  const content = isAbout
    ? `
      <p class="note-content">
        We are 3rd year students pursuing Bachelor's in Computer Science, and this is our PBL
        project for Web Development.
      </p>
      <div class="status-box">
        <strong>Team QuadCore</strong>
        <p class="tiny-copy">Ayushi Kandari, Abhay Chauhan, Suhani Saklani, Vanshika Panwar</p>
      </div>
    `
    : `
      <p class="note-content">
        Students and teachers can contact Team QuadCore directly through the email IDs below.
      </p>
          ...
          .map(
            (member) => `
              <article class="contact-card">
                ...
                <a class="inline-link" href="mailto:${escapeHtml(member.email)}">${escapeHtml(member.email)}</a>
              </article>
            `
          )
          .join("")}
  ...
  const modal = document.createElement("div");
  modal.className = "info-modal-shell";
  modal.innerHTML = `
    <div class="note-modal-backdrop" data-close-info="true"></div>
    <section class="note-modal glass-card">
      <div class="resource-head">
        <div>
          <span class="eyebrow">Team QuadCore</span>
          <h3>${title}</h3>
        </div>
        <button class="secondary-button" type="button" data-close-info="true">Close</button>
      </div>
      <div class="note-modal-body">${content}</div>
    </section>
  `;
  document.body.appendChild(modal);
  modal.addEventListener("click", (event) => {
...
authAboutButton?.addEventListener("click", () => openInfoModal("about"));
authHelpButton?.addEventListener("click", () => openInfoModal("help"));
dashboardAboutButton?.addEventListener("click", () => openInfoModal("about"));
dashboardHelpButton?.addEventListener("click", () => openInfoModal("help"));
